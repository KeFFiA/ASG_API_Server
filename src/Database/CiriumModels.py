...
class CiriumAircraftsDeltas(Base):
    source_id: Mapped[int] = mapped_column(
        ForeignKey("ciriumaircraft.id", ondelete="CASCADE"),
        nullable=False,
        unique=True,
        index=True
    )
    ...
    revision_id: Mapped[int] = mapped_column(
        ForeignKey("aircraftrevisions.id", ondelete="CASCADE"),
        nullable=False,
        index=True
    )
